...
def get_sudoku_dataset(cache_dir, streaming=False, mode='train'):
    if streaming:
        raise NotImplementedError('Streaming sudoku dataset is not implemented yet.')

    sudoku_dir = os.path.join(cache_dir, 'sudoku')
    train_path = os.path.join(sudoku_dir, 'easy_sudokus_answer_train.csv')
    validation_path = os.path.join(sudoku_dir, 'easy_sudokus_answer_valid.csv')
    test_path = os.path.join(sudoku_dir, 'hard_sudokus_valid.csv')

    column_names = [f"col{i}" for i in range(89)]
    ...
    # convert each row into a list of ints under key 'input'
    dataset = dataset.map(lambda row: {'input_ids': [row[f'col{i}'] for i in range(89)]})
    return dataset[mode]
...
def get_dataset(
  ...
  if dataset_name == 'sudoku':
    # For sudoku dataset, we don't need tokenization
    dataset = get_sudoku_dataset(cache_dir=cache_dir, streaming=False, mode=mode)
...
def get_tokenizer(config):
  if config.data.tokenizer_name_or_path == 'sudoku':
    # For sudoku dataset, we don't need a tokenizer
...
def get_dataloaders(config, tokenizer, skip_train=False,
  ...
  if skip_valid:
    ...
    if config.data.valid == 'sudoku':
      persistent_works_valid = True
    else:
      persistent_works_valid = False
    valid_loader = torch.utils.data.DataLoader(
      ...
      generator=generator,
      drop_last=False,
      persistent_workers=persistent_works_valid)
