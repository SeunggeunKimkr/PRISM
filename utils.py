...
@rank_zero_only
def log_target_pred_pair(self, name, target, pred, max_points=2048):
    """
    Log target vs. prediction for debugging.
    Assumes `target` and `pred` share the same (B, L[, ...]) shape.
    `mask` is a boolean tensor (B, L) indicating which positions were trained on.
    """
    tgt = target.detach().float().flatten()
    prd = pred.detach().float().flatten()
    if tgt.numel() == 0:
        return
    if tgt.numel() > max_points:
        idx = torch.randperm(tgt.numel(), device=tgt.device)[:max_points]
        tgt = tgt[idx]
        prd = prd[idx]
    mse  = F.mse_loss(prd, tgt)
    corr = torch.corrcoef(torch.stack([tgt, prd]))[0, 1].item()
    tgt_np = tgt.cpu().numpy()
    prd_np = prd.cpu().numpy()
    wb = getattr(self.logger, "experiment", None)
    if wb is None:
        return  # not using wandb
    import wandb
    table = wandb.Table(data=list(zip(tgt_np, prd_np)), columns=["target", "pred"])
    wb.log({
        f"debug/{name}_mse": mse.item(),
        f"debug/{name}_spearman": float(np.corrcoef(
            tgt_np, prd_np)[0,1]),
        f"debug/{name}_corrcoef": corr,
        f"debug/{name}_hist_target": wandb.Histogram(tgt_np),
        f"debug/{name}_hist_pred": wandb.Histogram(prd_np),
        f"debug/{name}_scatter": wandb.plot.scatter(
            table, "target", "pred", title=f"{name}: target vs pred"
        )
    })

class ParamGroupLRLogger(Callback):
    def on_fit_start(self, trainer, pl_module):
        names = []
        if getattr(pl_module.config, "adapter", None) and pl_module.config.adapter.enable:
            if getattr(pl_module.config.adapter_training, "tune_backbone", False):
              names.append("backbone")
            names.append("adapter")
            names.append("noise")
        else:
            names.extend(["backbone", "noise"])
        pl_module._lr_group_names = names

    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        opt = trainer.optimizers[0]
        lrs = [pg["lr"] for pg in opt.param_groups]
        pl_module.log_dict({f"lr/{n}": v for n, v in zip(pl_module._lr_group_names, lrs)},
